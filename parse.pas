...
//  "2022-10-30t07:00:00z"
  ...
  if suffix <> 0 then
  begin
    if length(value.SubString(20, 5)) < 5 then
      raise Exception.Create('Invalid suffix ' + value.SubString(19, 6));
...
begin
  ...
  begin
    s := sl[1].Substring(pos(':', sl[1]) + 1);
    parseDate(s, FFrom);
  end;
  if Pos('"interval_end"', sl[2]) <> 0 then
  begin
    s := sl[2].Substring(pos(':', sl[2]) + 1);
    parseDate(s, FTill);
  end;
  sl.Free;
end;
